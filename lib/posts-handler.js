  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        //decodedはcontent=投稿内容 => dataArray[0]
        //   &oneTimeToken=トークン本体 => dataArray[1]
        //という形式になっているのでまず、'&'で分割してやる
        const dataArray = decoded.split('&');
        //content(投稿内容を保存する変数)
        // 投稿内容があればcontent=の後の投稿内容の部分を取得、なければ空白を取得
        const content = dataArray[0] ? dataArray[0].split('content=')[1] : '';
        //requestedOneTimeToken(ワンタイムトークンを保存する変数)
        //ワンタイムトークンがあればoneTimeToken=の後ろのトークン本体を取得、なければ空白を取得
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
        //oneTimeTokenMap(ハッシュテーブル)に.get(登録されているトークン)と
        //requestedOneTimeToken(リクエストされてきたトークン)が同じなら
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
          //標準出力にcontent(新しく投稿された内容)を出力する
          console.info('投稿されました: ' + content);
          Post.create({
            content: content, //投稿内容
            trackingCookie: trackingId, //ハッシュ値を足して作成したID
            postedBy: req.user //投稿者
          }).then(() => {　//then(正常に投稿できたならば)
            //ハッシュテーブルから投稿ユーザーのワンタイムトークンを削除する
            oneTimeTokenMap.delete(req.user);
            //　/postsへリダイレクトする。直後に新たなワンタイムトークンが生成される。
            handleRedirectPosts(req, res);
          });
        } else {
          //正しいトークンでない場合は、400ステータスコード
